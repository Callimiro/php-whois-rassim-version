...
function lookup(string $domain)
{
    ...
    // Define the array of domains to process
    $domains = array();
    ...
    while (($data = fgetcsv($handle)) !== false) {
        // Extract the domain from the line and add it to the array
        $domains[] = $data[0];
    }
    ...
    fputcsv($fp, array('Domain', 'Name', 'Phone', 'Email'));

    $whois = Factory::get()->createWhois();

    // Loop through each domain and extract the desired data
    foreach ($domains as $domain) {
        // Perform WHOIS lookup and extract the desired data
        ...
        $phone = '';
        $email = '';
        //Registrant Name,Registrant Phone,Registrant Email:
        //$name =getStringBetween($result, 'Registrant Name: ', 'Registrant Organization:');

        // Search for the Registrant Email in the "text" property of the "data" array
        if (preg_match('/Registrant Email:\s*([^\s]+)/i', $data, $matches)) {
            $email = $matches[1];
        }
        /*     if (preg_match('/Registrant Name:\s*([^\s]+)/i', $data, $matches)) {
        $name = $matches[1];
    } */
        if (preg_match('/Registrant Phone:\s*([^\s]+)/i', $data, $matches)) {
            $phone = $matches[1];
        }

        // Find the position of the "Registrant Name:" string in the text
        $pos = strpos($data, "Registrant Name:");

        if ($pos !== false) {
            // Extract the name that appears after the "Registrant Name:" string
            $start = $pos + strlen("Registrant Name:");
            $end = strpos($data, "\n", $start);
            $registrantName = trim(substr($data, $start, $end - $start));
            $name =  $registrantName;
        }
